...
export default function TabelaClientes({
  ...
  const { search, statusFilter, tipoFilter, localFilter } = useTabelaClientesStore();
  ...
  const clientesFiltrados = useMemo(() => {
    return clientes.filter((cliente) => {
      ...
      const matchTipo = tipoFilter === "all" || cliente.secureType === tipoFilter;
      const matchLocal = localFilter === "all" || cliente.location === localFilter;
        ...
        <table className="w-full min-w-full  ">
          <thead>
            <tr className="border-b border-slate-700">
              ...
              <th className="pb-3 text-left text-xs font-medium tracking-wider text-slate-400">
                Renovação
              </th><th className="pb-3 text-left text-xs font-medium tracking-wider text-slate-400">
                Região
              </th>
            </tr>
          </thead>
